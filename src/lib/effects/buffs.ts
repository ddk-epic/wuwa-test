import type { BuffDefinition } from "@/constants/types"

export const buffs: Record<string, BuffDefinition[]> = {
  encore: [
    {
      id: "Angry Cosmos",
      name: "Angry Cosmos",
      type: "Buff",
      source: "encore",
      trigger: { skill: ["Cosmos Rave"] },
      appliesTo: "encore",
      modifiers: [{ class: "all", value: 0.1 }],
      duration: 10,
    },
    {
      id: "Woolies Cheer Dance",
      name: "Woolies Cheer Dance",
      type: "Buff",
      source: "encore",
      trigger: { skill: ["Flaming Woolies", "Cosmos Rampage"] },
      appliesTo: "encore",
      modifiers: [{ class: "fusion", value: 0.1 }],
      duration: 10,
    },
    {
      id: "Cosmos Rave",
      name: "Cosmos Rave",
      type: "Mode",
      source: "encore",
      trigger: { skill: ["Cosmos Rave"] },
      appliesTo: "encore",
      modifiers: [],
      duration: 10,
    },
    {
      id: "Wooly's Fairy Tale",
      name: "Wooly's Fairy Tale",
      type: "Buff",
      source: "encore",
      trigger: { category: ["basic"] },
      appliesTo: "encore",
      modifiers: [{ class: "fusion", value: 0.03 }],
      stackLimit: 4,
      sequenceReq: 1,
      duration: 6,
    },
    {
      id: "Sheep-counting Lullaby",
      name: "Sheep-counting Lullaby",
      type: "DCondFlat",
      source: "encore",
      trigger: { type: "hit", category: ["basic"] },
      appliesTo: "encore",
      modifiers: [{ class: "resonance", value: 10 }],
      cooldown: 10,
      sequenceReq: 2,
      duration: 0,
    },
    {
      id: "Fog? The Black Shores!",
      name: "Fog? The Black Shores!",
      type: "Buff",
      source: "encore",
      trigger: { type: "hit", skill: ["Cloudy Frenzy", "Cosmos Rupture"] },
      appliesTo: "encore",
      modifiers: [{ class: "multiplier", value: 0.4 }],
      sequenceReq: 3,
      duration: 0,
    },
    {
      id: "Adventure? Let's go!",
      name: "Adventure? Let's go!",
      type: "Buff",
      source: "encore",
      trigger: { skill: ["Cosmos Rupture"] },
      appliesTo: "all",
      modifiers: [{ class: "fusion", value: 0.2 }],
      sequenceReq: 4,
      duration: 30,
    },
    {
      id: "Hero Takes the Stage!",
      name: "Hero Takes the Stage!",
      type: "Buff",
      source: "encore",
      appliesTo: "encore",
      modifiers: [{ class: "skill", value: 0.35 }],
      sequenceReq: 5,
      duration: 99999,
    },
    {
      id: "Woolies Save the World!",
      name: "Woolies Save the World!",
      type: "Buff",
      source: "encore",
      trigger: { type: "hit", skill: ["all"], mode: ["Cosmos Rave"] },
      appliesTo: "encore",
      modifiers: [{ class: "atk", value: 0.05 }],
      stackLimit: 5,
      stackInterval: 0,
      sequenceReq: 6,
      duration: 10,
    },
  ],

  sanhua: [
    {
      id: "Condensation",
      name: "Condensation",
      type: "Buff",
      source: "sanhua",
      trigger: { type: "hit", category: ["intro"] },
      appliesTo: "sanhua",
      modifiers: [{ class: "skill", value: 0.2 }],
      duration: 8,
    },
    {
      id: "Avalanche",
      name: "Avalanche",
      type: "Buff",
      source: "sanhua",
      trigger: { type: "hit", skill: ["Frigid Light 5"] },
      appliesTo: "sanhua",
      modifiers: [],
      duration: 8,
    },
    {
      id: "Avalanche (bonus)",
      name: "Avalanche (bonus)",
      type: "Buff",
      source: "sanhua",
      trigger: {
        type: "hit",
        skill: ["Detonate"],
        condition: ["Avalanche"],
      },
      appliesTo: "sanhua",
      modifiers: [{ class: "bonus", value: 0.2 }],
      duration: 0.1,
    },
    {
      id: "Silversnow",
      name: "Silversnow",
      type: "BuffNext",
      source: "sanhua",
      trigger: { skill: ["Silversnow"] },
      appliesTo: "next",
      modifiers: [{ class: "baDeep", value: 0.38 }],
      duration: 14,
    },
    {
      id: "Ice Prism",
      name: "Ice Prism",
      type: "Damage",
      source: "sanhua",
      classifications: ["glacio", "skill"],
      trigger: { type: "hit", skill: ["Eternal Frost"] },
      appliesTo: "sanhua",
      modifiers: [{ class: "glacio", value: 0.4, concerto: 15, resonance: 7 }],
      duration: 5,
    },
    {
      id: "Ice Thorn",
      name: "Ice Thorn",
      type: "Damage",
      source: "sanhua",
      classifications: ["glacio", "skill"],
      trigger: { type: "hit", skill: ["Freezing Thorns"] },
      appliesTo: "sanhua",
      modifiers: [{ class: "glacio", value: 0.3, resonance: 2 }],
      duration: 8,
    },
    {
      id: "Ice Glacier",
      name: "Ice Glacier",
      type: "Damage",
      source: "sanhua",
      classifications: ["glacio", "skill"],
      trigger: { type: "hit", skill: ["Glacial Gaze"] },
      appliesTo: "sanhua",
      modifiers: [{ class: "glacio", value: 0.7, concerto: 15, resonance: 7 }],
      duration: 5,
    },
    {
      id: "Solitude's Embrace",
      name: "Solitude's Embrace",
      type: "Buff",
      source: "sanhua",
      trigger: { type: "hit", skill: ["Frigid Light 5"] },
      appliesTo: "sanhua",
      modifiers: [{ class: "crit", value: 0.15 }],
      sequenceReq: 1,
      duration: 10,
    },
    // {
    //   id: "Anomalous Vision",
    //   name: "Anomalous Vision",
    //   type: "Buff",
    //   source: "sanhua",
    //   trigger: {},
    //   appliesTo: "sanhua",
    //   modifiers: [{ class: "all", value: 0.35 }],
    //   sequenceReq: 3,
    //   duration: 10,
    // },
    {
      id: "Blade Mastery",
      name: "Blade Mastery",
      type: "Buff",
      source: "sanhua",
      trigger: { skill: ["Glacial Gaze"] },
      appliesTo: "sanhua",
      modifiers: [],
      sequenceReq: 4,
      duration: 5,
    },
    {
      id: "Blade Mastery (energy)",
      name: "Blade Mastery (energy)",
      type: "DCondFlat",
      source: "sanhua",
      trigger: { skill: ["Glacial Gaze"] },
      appliesTo: "sanhua",
      modifiers: [{ class: "resonance", value: 10 }],
      sequenceReq: 4,
      duration: 0,
    },
    {
      id: "Blade Mastery (bonus)",
      name: "Blade Mastery (bonus)",
      type: "Buff",
      source: "sanhua",
      trigger: {
        type: "hit",
        skill: ["Detonate"],
        condition: ["Blade Mastery"],
      },
      appliesTo: "sanhua",
      modifiers: [{ class: "bonus", value: 1.2 }],
      sequenceReq: 4,
      duration: 0.1,
    },
    {
      id: "Unraveling Fate",
      name: "Unraveling Fate",
      type: "Buff",
      source: "sanhua",
      trigger: { type: "hit", skill: ["Detonate"] },
      appliesTo: "sanhua",
      modifiers: [{ class: "critDmg", value: 1, concerto: 7.5 }], // 2 Detonate hits
      sequenceReq: 5,
      duration: 0.1,
    },
    {
      id: "Daybreak Radiance",
      name: "Daybreak Radiance",
      type: "Buff",
      source: "sanhua",
      trigger: { type: "hit", skill: ["Detonate"] }, // TODO: actual proc logic
      appliesTo: "all",
      modifiers: [{ class: "atk", value: 0.1 }],
      stackLimit: 2,
      stackInterval: 0,
      sequenceReq: 6,
      duration: 20,
    },
  ],

  shorekeeper: [
    {
      id: "Self Gravitation",
      name: "Self Gravitation",
      type: "Buff",
      source: "shorekeeper",
      trigger: {
        mode: [
          "Outer Stellarealm",
          "Inner Stellarealm",
          "Supernal Stellarealm",
        ],
      },
      appliesTo: "shorekeeper",
      modifiers: [{ class: "er", value: 0.1 }],
      duration: 0,
    },
    {
      id: "Self Gravitation (rover)",
      name: "Self Gravitation (rover)",
      type: "Buff",
      source: "shorekeeper",
      trigger: {
        mode: [
          "Outer Stellarealm",
          " Inner Stellarealm",
          "Supernal Stellarealm",
        ],
      },
      appliesTo: "rover",
      modifiers: [{ class: "er", value: 0.1 }],
      duration: 0,
    },
    {
      id: "Binary Butterfly",
      name: "Binary Butterfly",
      type: "BuffNext",
      source: "shorekeeper",
      trigger: { skill: ["Binary Butterfly"] },
      appliesTo: "all",
      modifiers: [{ class: "allDeep", value: 0.15 }],
      duration: 14,
    },
    {
      id: "Outer Stellarealm",
      name: "Outer Stellarealm",
      type: "Mode",
      source: "shorekeeper",
      trigger: { skill: ["End Loop"] },
      appliesTo: "all",
      modifiers: [],
      duration: 30,
    },
    // {
    //   id: "Outer Stellarealm (heal)",
    //   name: "Outer Stellarealm (heal)",
    //   type: "Heal",
    //   source: "shorekeeper",
    //   trigger: {
    //     mode: [
    //       "Outer Stellarealm",
    //       "Inner Stellarealm",
    //       "Supernal Stellarealm",
    //     ],
    //   },
    //   appliesTo: "current",
    //   modifiers: [{ class: "heal", value: 0.012, flatValue: 220 }], // TODO: implement heal
    //   duration: 30,
    // },
    {
      id: "Inner Stellarealm",
      name: "Inner Stellarealm",
      type: "Mode",
      source: "shorekeeper",
      trigger: { category: ["intro"], mode: ["Outer Stellarealm"] },
      appliesTo: "all",
      modifiers: [],
      duration: 30,
    },
    {
      id: "Supernal Stellarealm",
      name: "Supernal Stellarealm",
      type: "Mode",
      source: "shorekeeper",
      trigger: { category: ["intro"], mode: ["Inner Stellarealm"] },
      appliesTo: "all",
      modifiers: [],
      duration: 30,
    },
    {
      id: "Inner Stellarealm (On-Field)",
      name: "Inner Stellarealm (On-Field)",
      type: "Buff",
      source: "shorekeeper",
      trigger: { skill: ["all"], mode: ["Inner Stellarealm"] },
      appliesTo: "all",
      modifiers: [{ class: "crit", statReq: "er", value: 0.01 }], // TODO
      duration: 30,
    },
    {
      id: "Supernal Stellarealm (On-Field)",
      name: "Supernal Stellarealm (On-Field)",
      type: "Buff",
      source: "shorekeeper",
      trigger: { skill: ["all"], mode: ["Inner Stellarealm"] },
      appliesTo: "all",
      modifiers: [{ class: "critDmg", statReq: "er", value: 0.01 }], // TODO
      duration: 30,
    },
  ],
}
